...
const InterviewLivePage = () => {
  ...
  const handleForceEnd = useCallback(async (session: InterviewSession) => {
    ...
    const confirmed = window.confirm(
      t('interview:interviewDetail.messages.confirmForceEnd', {
        defaultValue: 'B?n có ch?c ch?n mu?n k?t thúc ngay bu?i ph?ng v?n này không?',
      })
    );
    if (!confirmed) return;
    ...
    try {
      ...
      setError(t('common:messages.actionFailed', { defaultValue: 'Thao tác th?t b?i. Vui lòng th? l?i.' }));
    } finally {
  ...
  const columns = useMemo(
    ...
    [t, getLink, theme.palette.success.main, theme.palette.error.main, handleForceEnd, actionLoadingId]
  );
