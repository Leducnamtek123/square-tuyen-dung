...
const showcaseSource = readFileSync(join(__dirname, '../../components/auths/EmployerSignUpShowcase.tsx'), 'utf8');
...
describe('Employer Auth Layout (design-taste)', () => {
  it('includes promotional showcase panel on employer login and dedicated B2B showcase on registration', () => {
    ...
    expect(showcaseSource).toContain('AuthShowcasePanel');
    expect(showcaseSource).toContain('variant="employer"');
  });
