...
const HOME_PAGE_PATH = join(__dirname, '..', 'index.tsx');

describe('HomePage Animation Resilience', () => {
  ...
  it('imports GSAP helper utilities and registers plugins safely', () => {
    ...
    expect(source).toContain('safeScrollTriggerRefresh');
  });
  ...
  it('ensures all ScrollTrigger animations have once: true and clearProps: "all"', () => {
    ...
    expect(source).toContain('dependencies: [careerSections]');
    expect(source).toContain('revertOnUpdate: true');
  });

  it('refreshes ScrollTrigger when careerSections update', () => {
    expect(source).toContain('safeScrollTriggerRefresh(100)');
  });
});
