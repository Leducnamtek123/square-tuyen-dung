...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      title: `${doc.title} | Nhà tuyển dụng ${APP_NAME}`,
      description: doc.subtitle || doc.summary,
      url: `https://infohr.vn/employer/${cleanSlug}.html`,
      type: 'article',
    },
  };
}
...
export default async function EmployerLegalHtmlPage({ params }: PageProps) {
  const resolvedParams = await params;
  const rawSlug = resolvedParams?.slug || '';
  const cleanSlug = typeof rawSlug === 'string' ? rawSlug.replace(/\.html$/i, '') : '';
  const doc = getLegalDocument(cleanSlug);
  ...
  return (
    <EmployerLayout>
      <LegalPolicyViewer slug={cleanSlug} portal="employer" />
    </EmployerLayout>
  );
}
