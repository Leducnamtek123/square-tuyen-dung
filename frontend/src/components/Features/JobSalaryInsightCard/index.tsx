...
const JobSalaryInsightCard = ({ slug }: Props) => {
  ...
  const scopeLabel = t(`jobDetail.salaryInsightScope.${data.scope || 'none'}`, {
    defaultValue: t('jobDetail.salaryInsightScope.none', 'Not enough market data'),
  });
  const confidenceLabel = t(`jobDetail.salaryInsightConfidence.${data.confidence || 'none'}`, {
    defaultValue: t('jobDetail.salaryInsightConfidence.none', 'Not enough data'),
  });
  const positionLabel = t(`jobDetail.salaryInsightPosition.${data.salaryPosition || 'unknown'}`, {
    defaultValue: t('jobDetail.salaryInsightPosition.unknown', 'Not enough data to compare'),
  });
      ...
      <CardContent>
        <Stack spacing={2.5}>
          ...
          <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
            {[
              [t('jobDetail.salaryInsightListings', 'Listings'), String(data.count ?? 0)],
              [t('jobDetail.salaryInsightMarketRange', 'Market range'), salaryString(data.minSalary, data.maxSalary)],
              [t('jobDetail.salaryInsightMedian', 'Median'), formatMoney(data.medianSalary)],
              [t('jobDetail.salaryInsightTypicalBand', 'Typical band'), salaryString(data.p25Salary, data.p75Salary)],
            ].map(([label, value]) => (
              ...
              <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={1}>
                <Box>
                  <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 700 }}>
                    {t('jobDetail.salaryInsightCurrentOffer', 'This offer')}
                  </Typography>
                  <Typography variant="body2" sx={{ fontWeight: 900 }}>
                    {salaryString(data.currentSalaryMin, data.currentSalaryMax)}
                  </Typography>
                </Box>
                <Box sx={{ textAlign: { xs: 'left', sm: 'right' } }}>
                  <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 700 }}>
                    {t('jobDetail.salaryInsightComparison', 'Comparison')}
                  </Typography>
              ...
              {hasMarketData && minScale != null && maxScale != null ? (
                <Box>
                  ...
                  <Stack direction="row" justifyContent="space-between" sx={{ mt: 0.75 }}>
                    <Typography variant="caption" color="text.secondary">{formatMoney(minScale)}</Typography>
                    <Typography variant="caption" color="text.secondary">{formatMoney(maxScale)}</Typography>
                  </Stack>
                </Box>
              ) : (
                <Typography variant="body2" color="text.secondary">
                  {t('jobDetail.salaryInsightInsufficient', 'There is not enough comparable salary data yet.')}
                </Typography>
              )}
            </Stack>
          </Box>
          ...
          {hasMarketData && (
            <Typography variant="body2" color="text.secondary">
              {t('jobDetail.salaryInsightAverageRange', 'Average posted range')}: {salaryString(data.avgMinSalary ?? null, data.avgMaxSalary ?? null)}
            </Typography>
          )}
