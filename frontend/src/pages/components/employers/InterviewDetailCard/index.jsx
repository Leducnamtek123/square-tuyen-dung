...
import { Box, Typography, Paper, Divider, Chip, Button, CircularProgress, Stack, IconButton } from "@mui/material";
...
const InterviewDetailCard = () => {
    ...
    if (!session) return (
        <Box sx={{ textAlign: 'center', py: 5 }}>
            <Typography color="text.secondary">Interview session information not found</Typography>
            ...
            >
                <Box>
                    ...
                    <Typography variant="body2" color="text.secondary" sx={{ opacity: 0.8 }}>
                        Room Code: <Box component="span" sx={{ fontWeight: 600, color: 'primary.main' }}>{session.room_name}</Box> | ID: {session.id}
                    </Typography>
                </Box>
                <Chip
                ...
                <Grid
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                        <Paper sx={{
                            ...
                            <Typography variant="subtitle1" gutterBottom sx={{ fontWeight: 700 }}>Interview Information</Typography>
                            <Divider sx={{ my: 2 }} />
                            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                                <Box>
                                    <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>CANDIDATE</Typography>
                                ...
                                <Box>
                                    <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>POSITION</Typography>
                                    <Typography variant="body1" sx={{ fontWeight: 600, mt: 0.5 }}>{session.jobName || 'N/A'}</Typography>
                                </Box>
                                <Box>
                                    <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5 }}>INTERVIEW TYPE</Typography>
                                ...
                                <Box>
                                    ...
                                    <Typography variant="body2" sx={{ mt: 0.5, fontWeight: 500 }}>{new Date(session.scheduledAt).toLocaleString('en-US')}</Typography>
                                </Box>
                            </Box>
                        </Paper>
                        ...
                        <Paper sx={{
                            ...
                            <Typography variant="subtitle1" gutterBottom sx={{ fontWeight: 700 }}>AI Evaluation Result</Typography>
                            {session.ai_overall_score !== null && session.ai_overall_score !== undefined ? (
                                <Box sx={{ textAlign: 'center', py: 2 }}>
                                    ...
                                    <Typography variant="body2" sx={{ mt: 1.5, fontWeight: 500, color: 'text.secondary' }}>
                                        {session.ai_summary || 'AI evaluation results are being generated'}
                                    </Typography>
                                    <Button
                                        variant="contained"
                                        fullWidth
                                ...
                                <Box sx={{ py: 4, textAlign: 'center' }}>
                                    ...
                                    <Typography variant="body2" color="text.secondary" sx={{ fontWeight: 500 }}>
                                        {session.status === 'completed' ? 'AI is analyzing the conversation...' : 'Interview has not ended'}
                                    </Typography>
                                </Box>
                            )}
                        </Paper>
                ...
                <Grid
                    ...
                    <Paper sx={{
                        ...
                        <Box sx={{
                            ...
                            <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>Conversation History</Typography>
                        ...
                        <Box sx={{
                            ...
                            {Array.isArray(session.transcripts) && session.transcripts.length > 0 ? session.transcripts.map((t, index) => (
                                <Box key={index} sx={{
                                    ...
                                    <Typography variant="caption" sx={{
                                        ...
                                        {t.speaker_role === 'ai_agent' ? 'AI Interviewer' : 'Candidate'} • {new Date(t.create_at).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                                    </Typography>
                                </Box>
                            )) : (
                                <Box sx={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: 0.5 }}>
                                    <Stack alignItems="center" spacing={1}>
                                        <Typography variant="body2">No conversation data available</Typography>
                                    </Stack>
                                </Box>
                            )}
                        </Box>
