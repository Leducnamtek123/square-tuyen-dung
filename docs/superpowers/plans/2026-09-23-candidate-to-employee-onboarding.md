...
- [x] **Step 1: Định nghĩa model `EmployeeOnboardingProcess` và `OnboardingTaskItem` trong `api/apps/hrm/models.py`**
...
- [x] **Step 2: Đăng ký các model mới vào Django Admin (`api/apps/hrm/admin.py`)**
  Tạo `EmployeeOnboardingProcessAdmin` và `OnboardingTaskItemInline` để hỗ trợ tra cứu trong Django Admin backend.

- [x] **Step 3: Chạy makemigrations tạo migration file `0009`**
...
- [x] **Step 4: Chạy migrate và xác minh database schema**
...
- [x] **Step 1: Viết test case ban đầu kiểm tra khởi tạo và chuyển đổi trạng thái onboarding (`api/apps/hrm/tests_onboarding.py`)**
...
- [x] **Step 3: Triển khai các phương thức nghiệp vụ trong `api/apps/hrm/services.py`**
...
- [x] **Step 4: Chạy lại test suite dịch vụ**
...
- [x] **Step 1: Khai báo Types trong `frontend/src/services/hrmService.ts`**
  Định nghĩa chính xác các interface và type union theo payload của DRF backend.

- [x] **Step 2: Thêm các methods gọi API vào `hrmService`**
  Thực hiện các phương thức HTTP GET, POST tương ứng các endpoints đã tạo.

- [x] **Step 3: Định nghĩa Query Keys và Hooks trong `frontend/src/views/hrmPages/hooks/useHrmQueries.ts`**
...
- [x] **Step 4: Kiểm tra linting TypeScript**
...
- [x] **Step 1: Xây dựng `OnboardingKpiCards.tsx`**
...
- [x] **Step 2: Xây dựng `OnboardingFilters.tsx`**
  Thanh công cụ lọc:
...
- [x] **Step 3: Kiểm tra hiển thị và tính toán state**
...
- [x] **Step 1: Xây dựng `OnboardingTableView.tsx`**
...
- [x] **Step 2: Xây dựng `OnboardingKanbanView.tsx`**
...
- [x] **Step 3: Cập nhật `OnboardingPage/index.tsx` kết nối bộ lọc và chế độ xem**
...
- [x] **Step 4: Kết nối Drawer vào trang chính `OnboardingPage/index.tsx`**
...
- [x] **Step 1: Chạy toàn bộ Unit Tests Backend**
  Run: `pytest api/apps/hrm/tests_onboarding.py -v`
  Expected: PASS toàn bộ test cases không có lỗi (6 passed).

- [x] **Step 2: Chạy linter & format backend**
  Run: `python -m py_compile apps/hrm/*.py`
  Expected: Không có cảnh báo hay lỗi biên dịch.

- [x] **Step 3: Chạy linter & type check frontend**
  Run: `pnpm exec tsc --noEmit` & `pnpm exec eslint src/views/hrmPages/OnboardingPage/`
  Expected: 0 errors, 0 warnings.

- [x] **Step 4: Kiểm thử thủ công toàn bộ luồng E2E**
  Xác thực:
