...
class CustomAdminSite(admin.AdminSite):
    ...
    def user_chart(self, request):
        ...
        if days <= 31:
            ...
            queryset1 = (
                queryset1.annotate(year=ExtractYear('create_at'))
                .values('year')
                .annotate(count=Count('id'))
                .order_by('year')
            )
            ...
            queryset2 = (
                queryset2.annotate(year=ExtractYear('create_at'))
                .values('year')
                .annotate(count=Count('id'))
                .order_by('year')
            )
    ...
    def job_post_chart(self, request):
        ...
        if days <= 31:
            ...
            queryset3 = (
                queryset3.annotate(year=ExtractYear('create_at'))
                .values('year')
                .annotate(count=Count('id'))
                .order_by('year')
            )
