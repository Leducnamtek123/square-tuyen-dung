...
from interview.models import InterviewSession, Question
from interview.services import build_interview_context, get_next_question_payload, append_transcript, update_interview_status
...
class InterviewServiceTests(TestCase):
    ...
    def test_get_next_question_payload_advances_cursor(self):
        q1 = Question.objects.create(text="Question 1")
        q2 = Question.objects.create(text="Question 2")
        self.session.questions.add(q1, q2)

        payload = get_next_question_payload(self.session, advance=True)
...
class InterviewCompatEndpointTests(TestCase):
    ...
    def test_context_endpoint_returns_payload(self):
        response = self.client.get(f"/api/interviews/{self.session.room_name}/context")
    ...
    def test_next_question_endpoint(self):
        ...
        response = self.client.post(
            f"/api/interviews/{self.session.room_name}/next-question",
            data={"advance": True},
            format="json",
        )
        self.assertEqual(response.status_code, 200)
