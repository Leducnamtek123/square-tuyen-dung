...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            ...
            default="all",
            help=(
                "Type of data to seed: locations, careers, accounts, jobs, "
                "interviews, banners, feedbacks, articles, square, all"
            ),
        )
    ...
    def _clear_square_demo_data(self):
        square_ids = list(
            Company.objects.filter(company_name__icontains="Square").values_list("id", flat=True)
        )
    ...
    def handle(self, *args, **options):
        ...
        if seed_type == "square":
            ...
            seed_jobs()
            seed_banners()
            seed_feedbacks(square_only=True)
