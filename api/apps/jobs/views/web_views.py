...
from shared.configs.messages import APPLICATION_STATUS_MESSAGES, ERROR_MESSAGES
...
class AdminStatisticViewSet(viewsets.ViewSet):
    ...
    def general_statistics(self, request):
        ...
        total_job_posts_pending = JobPost.objects.filter(status=var_sys.JOB_POST_STATUS[0][0]).count()
...
class AdminJobPostViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            serializer = self.get_serializer(page, many=True, fields=[
                ...
                'jobName', 'isHot', 'isUrgent', 'status', 'createAt', 'deadline',
    ...
    def approve_job(self, request, pk=None):
        ...
        job_post.status = var_sys.JOB_POST_STATUS[2][0]  # Đã duyệt

        job_post.save()

        # Notify employer (logic can be added here)
    ...
    def reject_job(self, request, pk=None):
        ...
        job_post.status = var_sys.JOB_POST_STATUS[1][0]  # Không duyệt
