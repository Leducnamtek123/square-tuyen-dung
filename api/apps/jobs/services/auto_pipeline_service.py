...
from apps.jobs.services.candidate_matching_service import match_and_source_candidates_for_job
    ...
    for activity in activities:
        try:
            ...
            if (score or 0) >= min_score and getattr(job_post, "auto_interview_enabled", True):
                if activity.status in (var_sys.ApplicationStatus.PENDING_CONFIRMATION, var_sys.ApplicationStatus.SUITABLE):
                    activity.status = var_sys.ApplicationStatus.SUITABLE
