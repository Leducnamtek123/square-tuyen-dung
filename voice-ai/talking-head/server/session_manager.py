...
class SessionManager:
    ...
    async def create_session(self, params: dict, sessionid: str = None) -> str:
        ...
        stale_sids = [
            ...
            if s is None or (getattr(s, 'quit_event', None) is not None and s.quit_event.is_set())
        ]
        ...
        if active_count >= self.max_session:
            # Thu hồi session cũ nhất để nhường tài nguyên cho ứng viên mới
            oldest_sid = next(iter(list(self.sessions.keys())))
            logger.info("Max session reached (%d/%d). Pruning oldest session %s", active_count, self.max_session, oldest_sid)
