...
class RTCManager:
    ...
    async def handle_offer(self, request):
        ...
        custom_sid = params.get('sessionid')
        try:
            sessionid = await session_manager.create_session(params, sessionid=custom_sid)
        ...
        try:
            ...
            logger.exception("Error during _create_pc_and_answer for session %s:", sessionid)
            session_manager.remove_session(sessionid)
            return web.Response(
                status=500,
                content_type="application/json",
